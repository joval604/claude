...
export default function EditTransactionForm({ transaction, onSave, onClose }: Props) {
  const [type, setType] = useState<"expense" | "income">(transaction.type);
  const [date, setDate] = useState(transaction.date);
  const [description, setDescription] = useState(transaction.description);
  const [amount, setAmount] = useState(String(transaction.amount));
  const [category, setCategory] = useState<string>(transaction.category);
  const [productLine, setProductLine] = useState<"pins" | "apparel" | "general">(transaction.productLine);
